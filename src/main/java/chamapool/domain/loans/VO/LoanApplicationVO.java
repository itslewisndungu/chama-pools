...
public record LoanApplicationVO(Integer loanId, Double amount, LoanApprovalStatus status) {
  public LoanApplicationVO(LoanApplication application) {
    this(application.loan().id(), application.loan().amount(), application.approvalStatus());
  }
}
